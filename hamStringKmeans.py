import random, time, sys, math, gc
# import time
# import sys
# import math
import numpy as np
...
def main():
	...
	kdata = pool.map(makeVector, seqFrags)
	pool.close()
	pool.join()
...
class kmeans(object):
	...
	# calculate new centroid and return distance between new and old
		...
		shift = getDistance(oldCentroid, self.clusters[idx]['centroid'])
		return shift
	...
	def fit(self, data):
		...
		self.allPoints = [{'val': data[i], 'distance': np.array([-1 for _ in range(self.numClusters)], dtype=np.int8), 'cluster': -1} for i in range(len(data))]
		self.initialize()
		...
		while True:
			...
			chunksize = 200000
			pointChunks = [self.allPoints[i:i+chunksize] for i in range(0, len(self.allPoints), chunksize)]
			...
			for chunk in pointChunks:
				...
				chunkResult = pool.map(func, chunk)
				pool.close()
				pool.join()
				tempPoints.extend(chunkResult)
				# gc.collect()
